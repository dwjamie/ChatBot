...
with st.sidebar:
    st.header("机器人配置")
    role = st.text_area(label="角色设定", placeholder="机器人需要扮演什么角色？")
    info = st.text_area(label="信息收集", placeholder="有哪些需要向用户收集的信息？")
    system_message = f"""===
参考背景知识：
{role}
===
...
if user_message := st.chat_input("你好！"):
    ...
    if call_function:
        ...
        with st.chat_message(name="assistant", avatar="🤖"):
            if assistant_message["name"] == "submit_form":
